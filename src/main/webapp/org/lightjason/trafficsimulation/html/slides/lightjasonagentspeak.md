...
* Consider every execution path!
  1. The plan always fails, if the phase duration is less than one second (early exit).
  2. If the plan duration is larger than or equal to one second, set light to `red` and set phase duration to passed value of variable `newduration`.
